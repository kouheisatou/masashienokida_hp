      ...
      <AdminShell>
        <div className="p-8 h-full">
          ...
          <div className="flex gap-4 h-[calc(100vh-220px)]">
            {/* List */}
            <div className="w-80 flex-shrink-0 bg-white rounded-xl shadow-sm overflow-y-auto">
              {loading ? (
                ...
                <ul className="divide-y divide-gray-100">
                  {contacts.map((c) => (
                    <li key={c.id}
                      ...
                      className={`p-4 cursor-pointer hover:bg-gray-50 transition-colors ${selected?.id === c.id ? 'bg-gray-50 border-l-2 border-gray-900' : ''}`}>
                ...
                <div className="p-6">
                  ...
                  <div className="mt-4">
                    <a href={`mailto:${selected.email}?subject=Re: ${encodeURIComponent(selected.subject)}`}
                      className="bg-gray-900 text-white text-sm px-4 py-2 rounded-lg hover:bg-gray-700 transition-colors inline-block">
                      返信する
                    </a>
                  </div>
                </div>
