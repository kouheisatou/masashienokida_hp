...
function SortableRow({
  ...
  return (
    <tr ref={setNodeRef} style={style} className="hover:bg-gray-50">
      ...
      <td className="px-6 py-4 text-gray-500">{item.release_year}</td>
      <td className="px-6 py-4">
        <span className={`px-2 py-1 rounded-full text-xs font-medium ${item.is_published ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
          {item.is_published ? '公開' : '非公開'}
        </span>
      </td>
      <td className="px-4 sm:px-6 py-4 text-right flex gap-2 sm:gap-3 justify-end">
        <Link href={`/discography/${item.id}`} className="text-blue-600 hover:underline">編集</Link>
...
export default function DiscographyListPage() {
  ...
  const handleDragEnd = useCallback(async (event: DragEndEvent) => {
    ...
    setSaving(true);
    await api.PUT('/discography/reorder', { body: payload as never });
    setSaving(false);
  }, [items]);
                ...
                <table className="w-full text-sm min-w-[450px]">
                  ...
                  <SortableContext items={items.map((i) => i.id)} strategy={verticalListSortingStrategy}>
                    <tbody className="divide-y divide-gray-100">
                      {items.map((d) => (
                        <SortableRow key={d.id} item={d} onDelete={handleDelete} />
                      ))}
                    </tbody>
                  </SortableContext>
                </table>
