...
export default function BlogEditPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('');
    try {
      const { error: apiError } = await api.PUT('/admin/blog/{id}', {
        params: { path: { id } },
        body: {
          title: form.title!,
    ...
    <AuthGuard>
      <AdminShell>
        <div className="p-8 max-w-3xl">
          <h1 className="text-2xl font-bold text-gray-900 mb-6">ブログ 編集</h1>
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm p-6 space-y-5">
            ...
            <div className="flex gap-6">
              <div className="flex items-center gap-2">
                <input type="checkbox" id="members_only" checked={form.members_only ?? false} onChange={(e) => set('members_only', e.target.checked)} className="rounded" />
